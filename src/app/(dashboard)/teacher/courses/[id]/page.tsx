    ...
    <div>
      ...
      <div className="flex items-start justify-between gap-4 sm:gap-6 mb-6 sm:mb-8">
        <div className="flex items-start gap-3 sm:gap-4 min-w-0 flex-1">
          <div className={`flex h-10 w-10 sm:h-12 sm:w-12 flex-shrink-0 items-center justify-center border ${t.border} font-mono text-base sm:text-lg ${t.text}`}>
            &#128218;
          </div>
          <div className="min-w-0">
            <h1 className={`font-mono text-lg sm:text-xl lg:text-2xl font-bold tracking-wider ${t.text} break-words`}>
              {course.title.toUpperCase()}
            </h1>
            <div className={`mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 sm:gap-3 font-mono text-[10px] sm:text-xs ${t.textMid} tracking-wider`}>
      ...
      {/* Tabs */}
      <div className={`flex gap-0 border-b ${t.border} overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0`}>
    ...
    <div>
      ...
      {showUploadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className={`w-full max-w-md max-h-[calc(100dvh-2rem)] overflow-y-auto border ${t.border} ${isDark ? 'bg-black' : 'bg-white'} p-5 sm:p-6`}>
            ...
            <div className="mb-4">
              ...
              <input ref={fileInputRef} type="file" onChange={handleFileChange} className={`w-full border ${t.border} bg-transparent ${t.text} font-mono text-base sm:text-xs p-2`} accept=".pdf,.doc,.docx,.ppt,.pptx,.txt,.jpg,.jpeg,.png,.gif,.mp4,.mov" />
            </div>
            <div className="mb-4">
              ...
              {existingLectures.length > 0 ? (
                <>
                  <select value={lectureName} onChange={(e) => setLectureName(e.target.value)} className={`w-full border ${t.border} ${isDark ? 'bg-black' : 'bg-white'} ${t.text} font-mono text-base sm:text-xs p-2 mb-2`}>
                  ...
                  <input type="text" value={lectureName} onChange={(e) => setLectureName(e.target.value)} placeholder="Type a new lecture name" className={`w-full border ${t.border} bg-transparent ${t.text} font-mono text-base sm:text-xs p-2`} />
                </>
              ) : (
                <input type="text" value={lectureName} onChange={(e) => setLectureName(e.target.value)} placeholder="Enter lecture name" className={`w-full border ${t.border} bg-transparent ${t.text} font-mono text-base sm:text-xs p-2`} />
              )}
            </div>
            {uploadError && <p className={`mb-4 font-mono text-xs ${isDark ? 'text-red-400' : 'text-red-600'}`}>{uploadError}</p>}
            <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
              <BrutalistButton isDark={isDark} variant="secondary" className="w-full sm:w-auto" onClick={() => { setShowUploadModal(false); setLectureName(""); setUploadError(""); if (fileInputRef.current) fileInputRef.current.value = ""; }} disabled={isUploading}>CANCEL</BrutalistButton>
              <BrutalistButton isDark={isDark} className="w-full sm:w-auto" onClick={handleUploadSubmit} disabled={isUploading || !fileInputRef.current?.files?.[0]}>{isUploading ? "UPLOADING..." : "UPLOAD"}</BrutalistButton>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Upload button */}
      <div className="mb-6 flex items-center justify-stretch sm:justify-end">
        <BrutalistButton isDark={isDark} onClick={handleUploadClick} disabled={isUploading} className="w-full sm:w-auto" iconBefore={
            ...
            <div key={lecName}>
              ...
              <div className="space-y-2">
                {lectureMaterials.map((material) => {
                  ...
                  return (
                    <BrutalistCard key={material.id} isDark={isDark} className="flex items-center gap-3 sm:gap-4 !p-3 sm:!p-4">
...
function StudentsTab({ courseId, invitationCode, isDark }: Readonly<StudentsTabProps>) {
  ...
  return (
    <div>
      ...
      <BrutalistCard isDark={isDark} className="mb-6 sm:mb-8 flex flex-wrap items-center gap-3 sm:gap-4">
        <div className="flex-1 min-w-0">
          <p className={`font-mono text-[10px] ${t.textDim} tracking-[0.25em] uppercase`}>INVITATION CODE</p>
          <p className={`mt-1 font-mono text-base sm:text-lg font-bold tracking-widest ${t.text} break-all`}>{invitationCode}</p>
        </div>
        <button onClick={handleCopy} title="Copy to clipboard"
          className={`flex h-9 w-9 flex-shrink-0 items-center justify-center border transition-colors ${copied ? (isDark ? 'border-green-500/40 text-green-400' : 'border-green-600/50 text-green-700') : `${t.border} ${t.textMid} ${isDark ? 'hover:text-white hover:border-white/30' : 'hover:text-black hover:border-black/25'}`}`}>
          {copied ? (
        ...
        <BrutalistButton isDark={isDark} variant="secondary" size="sm" disabled className="hidden sm:inline-flex">GENERATE NEW</BrutalistButton>
      </BrutalistCard>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {students.map((student) => (
            <BrutalistCard key={student.id} isDark={isDark} className="flex items-center gap-3 sm:gap-4 !p-3 sm:!p-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className={`sm:hidden mt-1 font-mono text-[9px] ${t.textDim}`}>ENROLLED {formatDate(student.enrolled_at).toUpperCase()}</p>
              </div>
              <p className={`hidden sm:block font-mono text-[10px] ${t.textDim} flex-shrink-0`}>ENROLLED {formatDate(student.enrolled_at).toUpperCase()}</p>
            </BrutalistCard>
          ))}
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3">
        <BrutalistButton isDark={isDark} variant="secondary" className="w-full sm:w-auto" onClick={() => setShowAssignModal(true)} iconBefore={
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" />
          </svg>
        }>ASSIGN SESSION</BrutalistButton>
        <BrutalistButton isDark={isDark} className="w-full sm:w-auto" onClick={() => setShowCreateModal(true)} iconBefore={
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {sessions.length === 0 ? (
        ...
        <div className="space-y-2">
          {sessions.map((session) => {
            ...
            const card = (
              <BrutalistCard key={session.id} isDark={isDark} className={`flex items-center gap-3 sm:gap-4 !p-3 sm:!p-4 ${isClickable ? `cursor-pointer ${isDark ? 'hover:border-white/30' : 'hover:border-black/25'}` : ''}`}>
