...
export default function StudentSessionPage() {
  ...
  if (error || !session) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center border border-black/12 p-12 max-w-md">
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-6 lg:px-16 py-12 lg:py-20 max-w-4xl">
        ...
        <div className="mb-8 relative">
          ...
          <h1 className="font-mono font-bold text-2xl lg:text-4xl text-black tracking-wider leading-tight">
            {session.title}
          </h1>
        </div>
        ...
        {phase === "reading" && session.audio_status === "ready" && session.audio_url && (
          <div className="mb-10 border border-black/12 p-6 bg-black/[0.02]">
            ...
            {questions.length > 0 && (
              <div className="flex justify-center pt-8 pb-12">
                <button
                  ...
                  className="relative font-mono text-sm text-black border-2 border-black px-8 py-3 hover:bg-black hover:text-white transition-all duration-200 group"
                >
                  <span className="absolute -top-1 -left-1 w-2.5 h-2.5 border-t border-l border-black opacity-0 group-hover:opacity-100 transition-opacity" />
                  <span className="absolute -bottom-1 -right-1 w-2.5 h-2.5 border-b border-r border-black opacity-0 group-hover:opacity-100 transition-opacity" />
          ...
          <div>
            <div className="mb-10 text-center border-t border-b border-black/12 py-8">
              <div className="flex items-center justify-center gap-2 mb-3 opacity-60">
                ...
                <div className="flex-1 h-px bg-black/40" />
                <span className="font-mono text-[10px] text-black/50 tracking-[0.3em] uppercase">QUIZ</span>
                <div className="flex-1 h-px bg-black/40" />
                <span className="font-mono text-[10px] text-black/60 tracking-wider">002</span>
                <div className="w-8 h-px bg-black/40" />
              </div>
            ...
            <div className="mt-10 flex justify-center pb-12">
              <button
                ...
                className="relative font-mono text-sm text-black border-2 border-black px-8 py-3 hover:bg-black hover:text-white transition-all duration-200 group disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-black"
              >
                <span className="absolute -top-1 -left-1 w-2.5 h-2.5 border-t border-l border-black opacity-0 group-hover:opacity-100 transition-opacity" />
                <span className="absolute -bottom-1 -right-1 w-2.5 h-2.5 border-b border-r border-black opacity-0 group-hover:opacity-100 transition-opacity" />
        ...
        {phase === "results" && (
          <div>
            ...
            >
              ...
              <div className="font-mono text-[10px] tracking-[0.3em] uppercase mb-4 opacity-60">
                FINAL SCORE
              </div>
              <p className="font-mono text-6xl font-bold text-black tracking-wider">
            ...
            {quizResults.length === 0 && (
              <p className="text-center font-mono text-xs text-black/60 tracking-wider mb-12">
            ...
            <div className="flex justify-center pb-12">
              <Link
                href={`/student/courses/${courseId}`}
                className="relative inline-flex items-center gap-2 font-mono text-sm text-black border-2 border-black px-8 py-3 hover:bg-black hover:text-white transition-all duration-200 group"
              >
                <span className="absolute -top-1 -left-1 w-2.5 h-2.5 border-t border-l border-black opacity-0 group-hover:opacity-100 transition-opacity" />
                <span className="absolute -bottom-1 -right-1 w-2.5 h-2.5 border-b border-r border-black opacity-0 group-hover:opacity-100 transition-opacity" />
                ← BACK TO COURSE
            ...
            >
              <span
                className={`flex h-8 w-8 shrink-0 items-center justify-center border font-mono text-xs font-bold transition-all ${
                  isSelected
                    ? "border-black bg-black text-white"
                    : "border-black/30 bg-transparent text-black/60"
                }`}
              >
                {opt.key.toUpperCase()}
              </span>
...
function ResultQuestionCard({
  ...
  return (
    <div className="relative border border-black/12 bg-black/[0.02] p-6">
      <div className="absolute top-0 left-0 w-3 h-3 border-t border-l border-black/30" />
      <div className="absolute bottom-0 right-0 w-3 h-3 border-b border-r border-black/30" />
